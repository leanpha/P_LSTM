...
def lstm_model_fn(features, labels, mode, params):
    ...
    eval_metrics_op = {
        'accuracy': tf.metrics.accuracy(tf.argmax(onehot_labels, 1), tf.argmax(logits, 1))
    }
    return tf.estimator.EstimatorSpec(mode, pred, loss, train_op, eval_metrics_op)


classifier = tf.estimator.Estimator(model_fn=lstm_model_fn, model_dir='senti_lstm_ver2', params=params)
